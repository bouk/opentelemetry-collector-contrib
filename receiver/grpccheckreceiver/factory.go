...
var errConfigNotGRPCCheck = errors.New("config was not a gRPC check receiver config")

// NewFactory creates a factory for the gRPC check receiver.
...
func createMetricsReceiver(_ context.Context, params receiver.Settings, rConf component.Config, consumer consumer.Metrics) (receiver.Metrics, error) {
	cfg, ok := rConf.(*Config)
	if !ok {
		return nil, errConfigNotGRPCCheck
	}

	grpccheckScraper := newScraper(cfg, params)
	s, err := scraper.NewMetrics(grpccheckScraper.scrape, scraper.WithStart(grpccheckScraper.start), scraper.WithShutdown(grpccheckScraper.shutdown))
	if err != nil {
		return nil, err
	}
